...
class _HomePageState extends State<HomePage> {
  ...
  void initState() {
    ...
    Future.delayed(const Duration(seconds: 1), () {
      TFLiteWeb.initializeUsingCDN().then((value) async {
        _tfLieModel = await TFLiteModel.fromUrl(
          '/assets/assets/ai/safety_model.tflite',
        );
        setState(() {});
      }).catchError((e) {
        print(e);
  ...
  void _runModel() async {
    ...
    final result = _tfLieModel!.predict<List<dynamic>>(tensor);

    print('${(result[0] as List).length}');
