...
class DashboardPage extends StatefulWidget {
  ...
  @override
  State<DashboardPage> createState() => _DashboardViewState();
}

class _DashboardViewState extends State<DashboardPage> {
  final DashboardService _service = DashboardService();
  List<RiskMetric> _metrics = [];
  ...
  Future<void> _refreshData() async {
    final data = await _service.calculateAreaRisks();
    if (mounted) {
      setState(() {
        _metrics = data;
        _isLoading = false;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text("Safety Heatmap | تحليل المخاطر", style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
              ...
              child: SingleChildScrollView(
                ...
                child: Column(
                  ...
                  children: [
                    _buildChartCard(),
                    const SizedBox(height: 20),
                    const Text("Area Breakdown | تفاصيل المناطق", style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
                    const SizedBox(height: 10),
                    _buildAreaList(),
                  ],
                ),
  ...
  Widget _buildChartCard() {
    return Container(
      ...
      child: Column(
        children: [
          const Text("Risk Score Comparison", style: TextStyle(color: Colors.grey, fontWeight: FontWeight.w500)),
          const SizedBox(height: 25),
          Expanded(
            child: BarChart(
              BarChartData(
                alignment: BarChartAlignment.spaceAround,
                maxY: _metrics.isEmpty ? 10 : _metrics.map((m) => m.totalRiskScore).reduce((a, b) => a > b ? a : b) + 5,
                  ...
                  bottomTitles: AxisTitles(
                    sideTitles: SideTitles(
                      ...
                      getTitlesWidget: (value, meta) {
                        if (value.toInt() >= _metrics.length) return const SizedBox();
                        return Padding(
                          padding: const EdgeInsets.only(top: 8),
                          child: Text(_metrics[value.toInt()].area.substring(0, min(4, _metrics[value.toInt()].area.length)), style: const TextStyle(fontSize: 10)),
                        );
                      },
                    ),
                  ),
                ...
                borderData: FlBorderData(show: false),
                barGroups: _metrics.asMap().entries.map((e) {
                  return BarChartGroupData(
                    x: e.key,
                    barRods: [
                      BarChartRodData(
                        toY: e.value.totalRiskScore,
                        color: e.value.totalRiskScore > 20 ? Colors.redAccent : AppColors.primary,
                        width: 18,
        ...
        return Card(
          ...
          child: ListTile(
            leading: Container(
              ...
              height: 45,
              decoration: BoxDecoration(
                color: (m.totalRiskScore > 20 ? Colors.red : Colors.orange).withOpacity(0.1),
                shape: BoxShape.circle,
              ),
              child: Center(
                child: Text(m.totalRiskScore.toInt().toString(), style: TextStyle(fontWeight: FontWeight.bold, color: m.totalRiskScore > 20 ? Colors.red : Colors.orange)),
              ),
            ),
            title: Text(m.area, style: const TextStyle(fontWeight: FontWeight.bold)),
            subtitle: Text("Reports: ${m.reportCount} | Avg Severity: ${m.avgSeverity.toStringAsFixed(1)}"),
          ),
        );
      },
    );
