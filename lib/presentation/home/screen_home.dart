...
class _ScreenHomeState extends State<ScreenHome> {
  ...
  void _onObservationChanged(String text) {
    ...
    _debounce = Timer(const Duration(milliseconds: 1000), () async {
      ...
      try {
        ...
        setState(() {
          selectedType = _matchItem(["Unsafe_Condition", "Unsafe_Behavior", "NM", "FA"], classification['type']);
          selectedLevel = _matchItem(["Low", "Medium", "High"], classification['level']);
  ...
  void _checkDuplicates(List<double> newVector) {
    if (newVector.isEmpty || _existingReports.isEmpty) return;

    double maxSim = 0.0;
    String similarText = "";

    // Use the ServiceAI instance to access the math logic
    final detector = ServiceAI().duplicateDetector;

    for (var report in _existingReports) {
      List<double> existingVec = (report['vector'] as List).cast<double>();
      double sim = detector.calculateSimilarity(newVector, existingVec);
      if (sim > maxSim) {
        maxSim = sim;
        similarText = report['text'] ?? "";
      }
    }

    // Threshold: 85% similarity triggers a warning
    if (maxSim > 0.85) { 
      setState(() {
        _similarityScore = maxSim;
        _duplicateWarning = "${t('form_dup_found')}: \"${similarText.length > 50 ? '${similarText.substring(0, 50)}...' : similarText}\"";
      });
    }
  }

  String? _matchItem(List<String> options, String? value) {
    if (value == null) return null;
    try {
      return options.firstWhere((e) => e.toLowerCase() == value.toLowerCase());
    } catch (_) {
      return null;
    }
  }
